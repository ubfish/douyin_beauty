...
# 需要替换自己的APP信息
APP_ID = '370186'
API_KEY = 'SQu6e20UbGRoNtdQhk17Tf'
SECRET_KEY = 'UUy6LiYydR94LeXL3PoYXazusDsVm'
...
def analysis_face(face_list):
    ...
    if face_list:
        ...
        for face in face_list:
            ...
            result += '</br>识别到第%d张人脸：'% i
            i += 1
            # 判断是男、女
            if face['gender']['type'] == 'female':
                age = face['age']
                beauty = face['beauty']
                result += "性别：女，年龄：%d，颜值：%d." % (age, beauty)
                if 18 <= age <= 30 and beauty >= 70:
                    # 满足条件，可以进行点赞、评论了
                    result += '<span style="color:red">颜值及格，满足条件！</span>'
